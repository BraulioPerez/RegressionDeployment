...
def upload_file():
    ...
    if file and allowed_file(file.filename):
        ...
        # Procesar el archivo CSV cargado (por ejemplo, leerlo en un DataFrame)
        # Puedes realizar procesamiento o análisis adicional en 'df' si es necesario.
...
def clean_file():
    # Cargar la dirección del archivo
    filename = os.path.join(app.config['UPLOAD_FOLDER'], 'housing.csv')
    
    # Cargar los valores de q1 y q3 para la limpieza desde el formulario HTML
    q1_value = float(request.form["q1_value"]) * 0.01
    q3_value = float(request.form["q3_value"]) * 0.01

    # Asignar el archivo housing.csv a nuestro DataFrame como df y nuestra clase
    df = LimpiezaDatos(datos=filename, q1=float(q1_value), q3=float(q3_value))
    # Aplicar la eliminación de nulos
...
def train_model():
    # Seleccionar el archivo de datos limpios
    current_directory = os.getcwd()
    file_path = os.path.join(current_directory, "dataset_muy_limpio.csv")

    # Extraer el tamaño del conjunto de prueba desde el formulario HTML
    test_size = float(request.form["test_size"]) * 0.01

    # Asignar el modelo a una variable y pasarle el método de entrenamiento
...
def predict():
    ...
    # Obtener valores del formulario HTML para realizar la predicción
